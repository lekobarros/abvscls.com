<template>
  <section class="relative" ref="root" data-scroll-section>
    ...
    <div class="relative z-10" @mouseenter="onImagesToggleOpacity(1)" @mouseleave="onImagesToggleOpacity(0)">
      <ProjectItem @onProjectEnter="setCurrentProjectView" v-for="(item, key) in projects" :key="`c-project-${key}`" :id="key" :title="item.website" :subtitle="item.subtitle || null" :description="item.short_description" :date="item.years" :url="item.url" :color="item.color"
